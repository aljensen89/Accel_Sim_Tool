est_clust_model_iid_sim <- function(feat_dur,feat_label,feat_stat,num_clust,
                                   dist_variant,beta_vector,iid_sigma){
  ## Grabbing file names
  ...
  for (i in 1:length(sim_files)){
    ...
    estlabel_betas[i,"iid_sigma"] <- summary(linear_nodecomp)$sigma
    estlabel_betas[i,"MSE"] <- mean(linear_nodecomp$residuals^2)
    
    # adding the parameter estimate to the dataframe
    for (j in 1:lm_results){
      estlabel_betas[i,j+10] <- broom::tidy(linear_nodecomp)[j,"estimate"]
      estlabel_betas[i,j+(10+lm_results)] <- broom::tidy(linear_nodecomp)[j,"std.error"]
    }
  }

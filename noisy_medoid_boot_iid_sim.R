noisy_medoid_boot_iid_sim <- function(feat_dur,feat_label,feat_stat,num_clust,
                                      dist_variant,boot_num,resamp_levels,
                                      beta_vector,iid_sigma){
  ## Grabbing file names
  ...
  colnames(estlabel_betas) <- c("Cohort","boot_count","prop_correct",
                                ...
                                "iid_sigma","Beta_0","Beta_Clust2","Beta_Clust3",
  ...
  for (i in 1:length(sim_files)){
    ...
    for (j in 1:boot_num){
      ...
      eps = rnorm(n = nrow(dat), mean = 0, sd = iid_sigma)
      ...
      estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,"iid_sigma"] <- summary(linear_nodecomp)$sigma
      #estlabel_betas[i,"MSE"] <- mean(mixed_nodecomp$residuals^2)
      
      # adding the parameter estimate to the dataframe
      for (k in 1:lm_results){
        estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,k+10] <- broom::tidy(linear_nodecomp)[k,"estimate"]
        #estlabel_betas[i,k+(5+lm_results)] <- broom::tidy(linear_nodecomp)[j,"std.error"]
      }
    }
  }

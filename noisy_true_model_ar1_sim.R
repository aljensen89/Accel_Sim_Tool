noisy_true_model_ar1_sim <- function(beta_vector,ar1_phi,ar1_sig_z){
  ## Grabbing file names
  ...
  colnames(truelabel_betas) <- c("Cohort","phi","MSE",
  ...
  for (i in 1:length(sim_files)){
    ...
    truelabel_betas[i,"phi"] <- coef(mixed_nodecomp$modelStruct$corStruct,unconstrained=FALSE)
    ...
    for (j in 1:length(fixed_effects)){
      ...
      truelabel_betas[i,j+(3+length(fixed_effects))] <- broom.mixed::tidy(mixed_nodecomp)[j,"std.error"]
    }
  }

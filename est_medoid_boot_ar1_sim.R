est_medoid_boot_ar1_sim <- function(feat_dur,feat_label,feat_stat,num_clust,
                                    dist_variant,boot_num,resamp_levels,
                                    beta_vector,ar1_phi,ar1_sig_z){
  ## Grabbing file names
  ...
  estlabel_betas <- data.frame(matrix(data=NA,
                                      nrow=(length(sim_files)*boot_num),
                                      ncol=16))
  colnames(estlabel_betas) <- c("Cohort","boot_count","prop_correct",
                                "clust1_correct","clust2_correct","clust3_correct",
                                "clust4_correct","clust5_correct","clust6_correct",
                                "phi","Beta_0","Beta_Clust2","Beta_Clust3",
                                "Beta_Clust4","Beta_Clust5","Beta_Clust6")
  estlabel_betas$Cohort <- rep(1:length(sim_files),each=boot_num)
  estlabel_betas$boot_count <- rep(1:boot_num,times=length(sim_files))
  ...
  for (i in 1:length(sim_files)){
    ...
    init_wide_buckets_matrix <- wide_buckets %>%
      dplyr::select( -id, -day) %>%
      as.matrix
    
    init_optimal_clusters <- ClusterR::Cluster_Medoids(init_wide_buckets_matrix,
                                                       clusters=num_clust)
    
    # Bootstrap loop
    for (j in 1:boot_num){
      # create the bootstrapped dataset
      boot_buckets_data <- resample(dat=wide_buckets,cluster=c("id","day"),replace=resamp_levels)
      
      # transform into a matrix for prediction.strength()
      wide_buckets_matrix <- boot_buckets_data %>%
        dplyr::select( -id, -day) %>%
        as.matrix
      
      # generate pam model using a priori known number of clusters
      optimal_clusters <- ClusterR::predict_Medoids(wide_buckets_matrix,
                                                    MEDOIDS=init_optimal_clusters$medoids)
      
      # merge pam cluster assignments back in with wide data
      boot_buckets_data$observed_cluster <- optimal_clusters$clusters
      
      cluster_match <- boot_buckets_data %>%
      ...
      avg_ts_design <- avg_ts_designed(boot_buckets_data,simulated_clusters)
      avg_ts_estimate <- avg_ts_estimated(boot_buckets_data)
      ...
      # the user 
      des_est_dist <- as.matrix(dist_function(avg_ts_design,avg_ts_estimate,
                                              variant=dist_variant))
      
      ## accounting for rounding errors - need all values to be >=0 for Hungarian
      ## matching algorithm
      des_est_dist[abs(des_est_dist-0)<0.000001] <- 0 
      
      hung_match <- clue::solve_LSAP(des_est_dist, maximum=FALSE)
      ...
      estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,"prop_correct"] <- sum(diag(cont_table_sorted))/sum(cont_table_sorted)
      estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,"clust1_correct"] <- diag(cont_table_sorted)["0-24 sedentary"]/rowSums(cont_table_sorted)["0-24 sedentary"]
      estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,"clust2_correct"] <- diag(cont_table_sorted)["0-6 active"]/rowSums(cont_table_sorted)["0-6 active"]
      estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,"clust3_correct"] <- diag(cont_table_sorted)["6-12 active"]/rowSums(cont_table_sorted)["6-12 active"]
      estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,"clust4_correct"] <- diag(cont_table_sorted)["12-18 active"]/rowSums(cont_table_sorted)["12-18 active"]
      estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,"clust5_correct"] <- diag(cont_table_sorted)["18-24 active"]/rowSums(cont_table_sorted)["18-24 active"]
      estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,"clust6_correct"] <- diag(cont_table_sorted)["0-24 active"]/rowSums(cont_table_sorted)["0-24 active"]
      ...
      mixed_nodecomp <- nlme::gls(TG_nodecomp ~ cluster2 + cluster3 + cluster4 + 
                                  ...
                                  control=nlme::glsControl(tolerance=1e-4))
      
      fixed_effects <- as.numeric(broom.mixed::tidy(mixed_nodecomp)$estimate)
      
      # adding the phi estimate to the dataframe
      estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,"phi"] <- coef(mixed_nodecomp$modelStruct$corStruct,unconstrained=FALSE)
      #estlabel_betas[i,"MSE"] <- mean(mixed_nodecomp$residuals^2)
      
      # adding the parameter estimate to the dataframe
      for (k in 1:length(fixed_effects)){
        estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,k+10] <- broom.mixed::tidy(mixed_nodecomp)[k,"estimate"]
        #estlabel_betas[i,k+(5+fixed_effects)] <- broom.mixed::tidy(mixed_nodecomp)[j,"std.error"]
      }
    }
  }

est_model_twoboot_ar1_sim <- function(feat_dur,feat_label,feat_stat,num_clust,
                                      dist_variant,boot_num,
                                      resamp_levels_1,resamp_levels_2,
  ...
  for (i in 1:length(sim_files)){
    ...
    for (j in 1:boot_num){
      ...
      boot_buckets_int <- resample(dat=wide_buckets,cluster=c("id","day"),replace=resamp_levels_1)
      boot_buckets_data <- resample(dat=boot_buckets_int,cluster=c("id","day"),replace=resamp_levels_2)
      ...
      boot_buckets_data$observed_cluster <- optimal_clusters$clustering
      ...
      n_subj = length(unique(simulated_clusters$id)) #number of subjects

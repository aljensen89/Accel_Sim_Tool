est_clust_model_cs_sim <- function(feat_dur,feat_label,feat_stat,num_clust,
  ...
  for (i in 1:length(sim_files)){
    ...
    estlabel_betas[i,"cs_rho"] <- coef(mixed_nodecomp$modelStruct$corStruct,unconstrained=FALSE)
    ...
    for (j in 1:fixed_effects){
      ...
      estlabel_betas[i,j+(10+fixed_effects)] <- broom.mixed::tidy(mixed_nodecomp)[j,"std.error"]
    }
  }

noisy_medoid_boot_cs_sim <- function(feat_dur,feat_label,feat_stat,num_clust,
  ...
  colnames(estlabel_betas) <- c("Cohort","boot_count","prop_correct",
                                ...
                                "cs_rho","Beta_0","Beta_Clust2","Beta_Clust3",
  ...
  for (i in 1:length(sim_files)){
    ...
    for (j in 1:boot_num){
      ...
      estlabel_betas[estlabel_betas$Cohort==i & estlabel_betas$boot_count==j,"cs_rho"] <- coef(mixed_nodecomp$modelStruct$corStruct,unconstrained=FALSE)
      #estlabel_betas[i,"MSE"] <- mean(mixed_nodecomp$residuals^2)
      
      # adding the parameter estimate to the dataframe
      for (k in 1:fixed_effects){
